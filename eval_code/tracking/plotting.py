...
def plot_tracks(transition_model, positions):
    ...
    if dim == 3:
        ax.plot(positions[0], positions[1], positions[2], label=f"{model_abbrev}", color=color)
    else:
        ax.plot(positions[0], positions[1], label=f"{model_abbrev}", color=color)
...
# Author: Fred Lydeard
def add_track_unc_stonesoup(transition_model, positions, variances, opacity=0.3, cred_level=0.95):
